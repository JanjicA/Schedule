$(document).ready(function(){
    ...
    $("#dugme").click(function(){
        ...
        if(!regexname.test(name) || name == ""){
            arrayError.push("-Name must start with upper case letter(A-Z)!");
        }
    
        if(!regexusername.test(username) || username == ""){
            arrayError.push("-Username must start with upper case letter and at least one number(0...9)!");
        }
    
        if(!regexemail.test(email) || email == ""){
            arrayError.push("-Email is not in regular form!");
        }
